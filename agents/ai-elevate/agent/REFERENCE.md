# ai-elevate — Reference Documentation
...
Always set `asAgentId: "ai-elevate"` in every tool call.
...
1. **Your own AGENTS.md** — Add learnings, refine your processes, document patterns that work
   - File: `/home/aielevate/.openclaw/agents/ai-elevate/agent/AGENTS.md`
...
```
echo "$(date '+%Y-%m-%d %H:%M') | ai-elevate | {what you improved} | {why}" >> /opt/ai-elevate/memory/improvements.log
```
...
kg.link("deal", "deal-001", "agent", "ai-elevate", "managed_by")
...
log_interaction(reader_email, "content", "feedback", feedback_text, agent_id="ai-elevate", org="ai-elevate")
```
...
2. Forward immediately: `sessions_send to gigforge-support: "BUG REPORT FORWARDED FROM ai-elevate: {details}"`
...
If a user, customer, or team member reports a bug to you:
1. Reply: "Thanks for reporting this. I'm forwarding it to our support team — they'll contact you shortly with a tracking number."
2. Forward immediately via sessions_send to gigforge-support: "BUG REPORT FORWARDED FROM ai-elevate: [full details]"
3. Never file bugs yourself. Never say a bug is fixed. Only support handles bug lifecycle.
...
|-----------|-------------|----------|
| Story selection | scraper (sources), researcher (depth assessment) | reviewer (standards) |
| Editorial calendar | content (capacity), publisher (distribution windows) | writer (workload) |
| Quality standards | reviewer (criteria), factchecker (accuracy bar) | editor (voice) |
| Strategic direction | researcher (trends), publisher (audience data) | monitor (pipeline health) |
...
send_email(to="recipient@example.com", subject="Subject", body="Body", agent_id="ai-elevate")
```
...
Outbound: POST /call/outbound?agent_id=ai-elevate&to_number={NUMBER}&greeting={TEXT}
