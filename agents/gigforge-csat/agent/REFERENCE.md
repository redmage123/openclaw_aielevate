# gigforge-csat — Reference Documentation
...
- `sessions_send` — message other agents (always set `asAgentId: "gigforge-csat"`)
...
2. **Analyze with fuzzy logic:**
   ...
   result = process_message(customer_message, sender=customer_email, org="gigforge")
   ```
...
4. **Take action within 15 minutes:**
   ...
   - If it requires engineering, message gigforge-engineer with URGENT priority
   - If it requires a product change, message gigforge
5. **Follow up:**
...
|-------|----------------|
| gigforge-support | Get ticket history, customer background |
| gigforge-engineer | Urgent bug fixes, technical root cause |
| gigforge | Authorization beyond your limits, strategic decisions |

### Reporting

**Daily:** Log all escalations and outcomes
```bash
echo "$(date '+%Y-%m-%d %H:%M') | {CUSTOMER} | {ISSUE} | {RESOLUTION} | {COMPENSATION} | {CSAT}" >> /opt/ai-elevate/gigforge/support/escalation-log.csv
```

**Weekly:** Send CSAT summary to gigforge via sessions_send:
...
### Ticket Logs
- Ticket log: `/opt/ai-elevate/gigforge/support/ticket-log.csv`
- CSAT log: `/opt/ai-elevate/gigforge/support/csat-log.csv`
- Escalation log: `/opt/ai-elevate/gigforge/support/escalation-log.csv`
...
kg.link("deal", "deal-001", "agent", "gigforge-csat", "managed_by")
...
2. Forward immediately: `sessions_send to gigforge-support: "BUG REPORT FORWARDED FROM gigforge-csat: {details}"`
