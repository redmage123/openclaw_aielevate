# gigforge-creative — Reference Documentation
...
kg.link("deal", "deal-001", "agent", "gigforge-creative", "managed_by")
...
2. Forward immediately: `sessions_send to gigforge-support: "BUG REPORT FORWARDED FROM gigforge-creative: {details}"`
...
If a user, customer, or team member reports a bug to you:
1. Reply: "Thanks for reporting this. I'm forwarding it to our support team — they'll contact you shortly with a tracking number."
2. Forward immediately via sessions_send to gigforge-support: "BUG REPORT FORWARDED FROM gigforge-creative: [full details]"
3. Never file bugs yourself. Never say a bug is fixed. Only support handles bug lifecycle.
...
send_email(to="recipient@example.com", subject="Subject", body="Body", agent_id="gigforge-creative")
```
...
Outbound: POST /call/outbound?agent_id=gigforge-creative&to_number={NUMBER}&greeting={TEXT}
