...
def sync_org(org_slug):
    ...
    if not db_path:
        log.info("  Unknown org: %s", extra={"org_slug": org_slug})
        return 0

    entities, relationships = get_kg_entities(db_path)
    if not entities:
        log.info("  %s: no entities", extra={"org_slug": org_slug})
        return 0
    ...
    if docs_to_ingest:
        ...
        for i in range(0, len(docs_to_ingest), 50):
            ...
            if "error" in result:
                errors += len(batch)
                log.info("  RAG batch error: {result['error']}")
            else:
    ...
    log.info("  %s: synced %s entities (%s errors), %s total, %s relationships", extra={"org_slug": org_slug, "synced": synced, "errors": errors})
    return synced
...
def show_status():
    ...
    for org in ORGS:
        s = state.get(org, {})
        log.info("  %s:", extra={"org": org})
        log.info("    Last sync: {s.get('last_sync', 'never")
        log.info("    Entities synced: {s.get('entities_synced', 0)}")
        log.info("    Total entities: {s.get('total_entities', 0)}")
        log.info("    Relationships: {s.get('total_relationships', 0)}")
        log.info("    Errors: {s.get('errors', 0)}")
