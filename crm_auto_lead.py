...
def add_lead(email, name, org="", source="email", subject=""):
    ...
    if is_internal(email):
        log.info("Skipping internal team member: %s", extra={"email": email})
        return False
    ...
    if existing:
        ...
        conn.commit()
        log.info("Updated existing lead: %s (contact #%s)", extra={"email": email})
        conn.close()
        ...
        conn.commit()
        log.info("New lead registered: %s <%s> (org: %s, source: %s)", extra={"name": name, "email": email, "org": org})
        conn.close()
...
def try_crm_push(email, name, org, source):
    ...
    try:
        ...
        if resp.status in (200, 201):
            log.info("  -> Also pushed to CRM API")
...
def list_leads():
    ...
    log.info("{'Email':<35} {'Name':<20} {'Org':<15} {'Source':<10} {'Status':<8} {'Count':<6} {'First Contact':<20}")
    print("-" * 120)
    for lead in leads:
...
def weekly_report():
    ...
    print("=" * 60)
    log.info("WEEKLY LEAD REPORT — {now.strftime('%Y-%m-%d")
    print("=" * 60)
    log.info("\nNew leads this week: {len(new_leads)}")
    log.info("Total leads: %s", extra={"total": total})

    if new_leads:
        log.info("\nNew leads:")
        for lead in new_leads:
            log.info("  - {lead['name']} <{lead['email']}> (org: {lead['org']}, source: {lead['source']})")
            if lead["subject"]:
                log.info("    Subject: {lead['subject']}")

    if by_source:
        log.info("\nLeads by source:")
        for row in by_source:
            log.info("  {row['source']}: {row['c']}")

    if by_org:
        log.info("\nLeads by organization:")
        for row in by_org:
            log.info("  {row['org']}: {row['c']}")

    if by_status:
        log.info("\nLeads by status:")
        for row in by_status:
            log.info("  {row['status']}: {row['c']}")
