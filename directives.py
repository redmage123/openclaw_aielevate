...
from exceptions import AiElevateError  # TODO: Use specific exception types, PlaneError, DatabaseError
...
def _archive_plane_tickets(subject: str, reason: str, archived: dict) -> None:
    """Mark matching Plane tickets as cancelled for the given project subject."""
    try:
...
def _archive_kg_entities(subject: str, reason: str, archived: dict) -> None:
    """Mark matching KG entities as cancelled for the given project subject."""
    try:
...
def _list_preserved_files(subject: str, archived: dict) -> None:
    """List files related to the project that are preserved (never deleted)."""
...
def archive_project(subject: str, reason: str = ""):
    ...
    _archive_plane_tickets(subject, reason, archived)
    _archive_kg_entities(subject, reason, archived)
    _list_preserved_files(subject, archived)
    return archived
