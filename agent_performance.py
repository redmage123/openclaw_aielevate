...
def collect_metrics():
    ...
    try:
        ...
        log.info("SLA metrics error: %s", extra={"e": e})
                ...
                with open(log_file) as f:
                    for line in f:
                        try:
                            ...
                            pass
            log.info("Collected %s response metrics", extra={"total_responses": total_responses})
    except (DatabaseError, Exception) as e:
        log.info("Response log error: %s", extra={"e": e})
...
def generate_report():
    ...
    if rows:
        ...
        for agent, compliance in rows:
            log.info("  {agent:30s} {compliance:.0f}%")
    ...
    if rows:
        ...
        for agent, avg_time in rows:
            log.info("  {agent:30s} {avg_time:.0f}s")
