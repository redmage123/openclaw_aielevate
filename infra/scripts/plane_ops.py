...
class Plane:
    ...
    def create_bug(self, app: str, title: str, description: str,
        ...
        print(f"[Plane] Filed {self.org}/BUG-{seq}: [{priority.upper()}] {full_title}")
        print(f"[Plane] Reporter: {reporter} -- only they can sign off")
        return result
    ...
    def set_state(self, project: str, issue_id: str, state: str) -> dict:
        ...
        print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue -> {state}")
        return result
    ...
    def assign_bug(self, project: str, issue_id: str, assignee: str) -> dict:
        ...
        if current_state_id == states.get("Backlog"):
            ...
            print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue assigned to {assignee}, moved to Todo")
        else:
            print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue assigned to {assignee}")
        return issue
    ...
    def submit_to_qa(self, project: str, issue_id: str, engineer: str,
        ...
        print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue submitted to QA by {engineer} -> In Review")
        return result
    ...
    def qa_pass(self, project: str, issue_id: str, qa_agent: str,
        ...
        print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue QA PASSED by {qa_agent} — awaiting reporter sign-off")
    ...
    def qa_fail(self, project: str, issue_id: str, qa_agent: str,
        ...
        print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue QA FAILED by {qa_agent} -> Reopened")
        return result
    ...
    def sign_off(self, project: str, issue_id: str, reporter: str,
        ...
        print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue SIGNED OFF by {reporter} -> Done")
        return result
    ...
    def reopen(self, project: str, issue_id: str, reporter: str,
        ...
        print(f"[Plane] {ORG_PREFIX.get(self.org, self.org.upper()[:3])}-{project} issue REOPENED by {reporter}")
        return result
    ...
    def create_issue(self, project: str, title: str, description: str = "",
        ...
        print(f"[Plane] Created {ticket_id}: {title}")
        return result
