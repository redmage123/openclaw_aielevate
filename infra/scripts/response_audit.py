...
def run_audit():
    ...
    if not responses:
        print("No agent email responses found in the past week.")
        return
    ...
    if gigforge_samples:
        ...
        print(f"Sending {len(gigforge_samples)} samples to gigforge-pm...")
        try:
            ...
            print(f"gigforge-pm response ({len(result.stdout)} chars): {result.stdout[:500]}")
    ...
    if techuni_samples:
        ...
        print(f"Sending {len(techuni_samples)} samples to techuni-pm...")
        try:
            env = {k: v for k, v in os.environ.items() if k != "CLAUDECODE"}
            result = subprocess.run(
                [
            ...
            print(f"techuni-pm response ({len(result.stdout)} chars): {result.stdout[:500]}")
            if result.returncode != 0:
    ...
    if not gigforge_samples and not techuni_samples:
        print("No samples to audit after classification.")
        return
