...
def process_feedback(customer_email: str, rating: int = 0, comments: str = "",
    ...
    # Distribute feedback to ALL agents
    feedback_summary = (
    ...
    # Update customer sentiment based on rating
    try:
        from customer_context import update_sentiment
        if rating >= 8:
            update_sentiment(customer_email, "positive", f"Feedback: {rating}/10", agent="feedback-system")
        elif rating >= 5:
            update_sentiment(customer_email, "neutral", f"Feedback: {rating}/10", agent="feedback-system")
        else:
            update_sentiment(customer_email, "at_risk", f"Low feedback: {rating}/10", agent="feedback-system")
    except Exception:
        pass
    ...
    try:
        ...
        ops_notify(priority,
                  f"Feedback from {customer_email}: {rating}/10 for {project}",
...
def get_feedback_summary(org: str = "", months: int = 3) -> dict:
    ...
    cur.execute(f"SELECT COUNT(*) as cnt, AVG(rating) as avg, MIN(rating) as min, MAX(rating) as max FROM customer_feedback {where}", params)
    stats = dict(cur.fetchone())

    cur.execute(f"SELECT rating, COUNT(*) as cnt FROM customer_feedback {where} GROUP BY rating ORDER BY rating", params)
    distribution = {r["rating"]: r["cnt"] for r in cur.fetchall()}

    cur.execute(f"SELECT COUNT(*) as cnt FROM customer_feedback {where} AND would_recommend=TRUE", params)
    recommend = cur.fetchone()["cnt"]

    cur.execute(f"SELECT what_to_improve FROM customer_feedback {where} AND what_to_improve IS NOT NULL AND what_to_improve != ''", params)
