...
Output ONLY the HTML content sections. No wrapper, no doctype, no head/body tags."""
    ...
    try:
        ...
        if not content or len(content) < 50 or "error" in content.lower()[:100] or "authenticate" in content.lower()[:200] or "401" in content[:100]:
            print(f"  Content generation returned error or empty, using fallback")
        ...
        print(f"Content generation failed for {org}: {e}")
        return _fallback_content(org)
...
def generate_drafts():
    ...
    for org, cfg in NEWSLETTERS.items():
        print(f"Generating {cfg['subject_prefix']} content...")
    ...
    print("Drafts ready. Use --preview to review or --send to distribute.")
...
def _fetch_list_members(list_addr, creds):
    ...
    try:
        ...
        print(f"  Failed to fetch list members: {e}")
        return []
    ...
    for org, cfg in NEWSLETTERS.items():
        ...
        if not draft_path.exists():
            ...
            if drafts:
                ...
                print(f"No draft found for {org}. Run --generate first.")
                continue
        ...
        if any(marker.lower() in html.lower() for marker in error_markers):
            print(f"  {cfg['subject_prefix']}: BLOCKED — draft contains error content. Fix and regenerate.")
            continue
        ...
        if content_match and len(content_match.group(1).strip()) < 200:
            print(f"  {cfg['subject_prefix']}: BLOCKED — content too short ({len(content_match.group(1).strip())} chars). Regenerate.")
            continue
        ...
        if preview:
            ...
            if not recipients:
                print(f"  {cfg['subject_prefix']}: No subscribers found!")
                continue
        ...
        for name, addr in recipients:
            ...
            try:
                ...
                print(f"  {cfg['subject_prefix']}: FAILED for {addr} — {e}")

        print(f"  {cfg['subject_prefix']}: sent to {sent_count}/{len(recipients)} recipients")
    ...
    for org, cfg in NEWSLETTERS.items():
        ...
        try:
            ...
            for m in data.get("items", []):
                ...
                print(f"  {m['name']:25s} {m['address']:35s} [{status}]")
        except Exception as e:
            print(f"  Error: {e}")
