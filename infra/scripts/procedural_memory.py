...
def record(org, agent_id, task_type, approach, outcome, effectiveness,
    ...
    conn.commit()
    conn.close()
    print(f"[record] {org}/{agent_id}: {task_type} -> {outcome} (eff={effectiveness})")
...
def update_pattern(task_type, pattern, success=True):
    ...
    print(f"[pattern] {task_type}: '{pattern}' updated ({status}, confidence={confidence:.2f})")
...
def cmd_report(args):
    ...
    """).fetchall()
    if not rows:
        print("[report] No procedures recorded yet.")
        return
    print("=== Procedural Memory Report ===\n")
    for tt, total, succ, fail, part, avg_e, max_e in rows:
        sr = (succ / total * 100) if total > 0 else 0
        print(f"  {tt}:")
        print(f"    Total: {total} | Success: {succ} | Failure: {fail} | Partial: {part}")
        print(f"    Success rate: {sr:.0f}% | Avg effectiveness: {avg_e:.2f} | Max: {max_e:.2f}")
    ...
    if patterns:
        ...
        for tt, pat, conf, sc, fc in patterns:
            print(f"  [{tt}] {pat} (confidence={conf:.2f}, {sc}S/{fc}F)")
    ...
    if agents:
        ...
        for aid, total, avg_e, succ in agents:
            sr = (succ / total * 100) if total > 0 else 0
            print(f"  {aid}: {total} procedures, {sr:.0f}% success, avg_eff={avg_e:.2f}")
    conn.close()
...
def cmd_analyze(args):
    ...
    for tt, approach, cnt, avg_eff in candidates:
        ...
        if not existing:
            ...
            new_patterns += 1
            print(f"  New pattern: [{tt}] {pattern}")
    ...
    for tt, approach, cnt in fail_candidates:
        ...
        if not existing:
            ...
            new_patterns += 1
            print(f"  Anti-pattern: [{tt}] {pattern}")

    conn.commit()
    conn.close()

    print(f"\n[analyze] Discovered {new_patterns} new patterns.")
...
def main():
    ...
    if args.report:
        ...
        if not results:
            print(f"No procedures found for task type '{args.task_type}'")
        else:
            for r in results:
                org, aid, approach, outcome, eff, lessons, tags, created = r
                print(f"  [{outcome}] eff={eff:.2f} by {aid}@{org}: {approach}")
                if lessons:
                    print(f"    Lessons: {lessons}")
    elif args.command == "best":
        result = best_approach(args.task_type)
        if result:
            print(f"Best approach for '{args.task_type}':")
            ...
            print(f"No successful procedures found for '{args.task_type}'")
    elif args.command == "patterns":
        pats = patterns_for(args.task_type)
        if not pats:
            print(f"No patterns found for '{args.task_type}'")
        else:
            for p in pats:
                print(f"  [{p['confidence']:.2f}] {p['pattern']} ({p['success_count']}S/{p['failure_count']}F)")
