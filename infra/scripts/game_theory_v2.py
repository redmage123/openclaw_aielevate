...
def get_win_rate(platform: str = None, months: int = 6) -> dict:
    ...
    cur.execute(f"SELECT outcome, COUNT(*) as cnt FROM bid_history {where} GROUP BY outcome", params)
    ...
    cur.execute(f"""SELECT strategy, outcome, COUNT(*) as cnt
        FROM bid_history {where} GROUP BY strategy, outcome""", params)
    strategy_stats = {}
    ...
    cur.execute(f"""SELECT
        CASE WHEN overall_score >= 70 THEN 'high' WHEN overall_score >= 40 THEN 'medium' ELSE 'low' END as tier,
        outcome, COUNT(*) as cnt
        FROM bid_history {where} GROUP BY tier, outcome""", params)
    tier_stats = {}
    for r in cur.fetchall():
        t = r["tier"]
    ...
    cur.execute(f"SELECT outcome, AVG(our_bid) as avg_bid FROM bid_history {where} AND our_bid > 0 GROUP BY outcome", params)
