...
def cleanup_stale_jobs(max_age_hours: int = 72):
    """Remove scraped jobs older than max_age_hours."""
    conn, cur = _get_db()
    cur.execute("DELETE FROM scraped_jobs WHERE scraped_at < NOW() - INTERVAL '%s hours'", (max_age_hours,))
    deleted = cur.rowcount
    conn.close()
    if deleted:
        LOG.info(f"Cleaned up {deleted} stale jobs (>{max_age_hours}h old)")
    return deleted
...
def scrape_freelancer(keywords: list, max_pages: int = 2) -> list:
    ...
    with sync_playwright() as p:
        ...
        for keyword in keywords:
            for page_num in range(1, max_pages + 1):
                url = f"https://www.freelancer.com/jobs/?keyword={keyword}&page={page_num}&project_type=fixed&sort_field=time_updated&sort_direction=desc"
                    ...
                    for card in cards:
                        try:
                            ...
                            if title:
                                ...
                                days_el = card.query_selector('.JobSearchCard-primary-heading-days, [data-days-left], .bid-deadline, time')
                                days_text = days_el.inner_text().strip().lower() if days_el else ""
                                if any(x in days_text for x in ["closed", "ended", "expired", "awarded"]):
                                    continue  # Skip closed jobs
...
def get_top_jobs(limit: int = 20, min_score: float = 30) -> list:
    """Get top-scored jobs from the last 48 hours only."""
    conn, cur = _get_db()
    cur.execute(
        "SELECT * FROM scraped_jobs WHERE score >= %s "
